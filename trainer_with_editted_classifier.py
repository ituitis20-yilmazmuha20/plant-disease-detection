...
class TrainerWithEdittedClassifier:
    ...
    def load_data(self):
        ...
        # disease label to plant name (example: 63 -> 'Tomato')
        ...
        self.label_to_onehot = {label: plant_to_onehot[plant] for label, plant in enumerate(plant_names)}
        # plant isminden
    ...
    def get_plant_types(self, labels):
        onehot_labels = [self.label_to_onehot[label.item()] for label in labels.cpu()]
        return torch.tensor(onehot_labels, dtype=torch.float32).to(self.device)
    ...
    def train_model(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for images, labels in self.train_loader:
                ...
                plant_types = self.get_plant_types(labels) # Get one-hot encoded plant types
                self.optimizer.zero_grad()
                outputs = self.model(images, plant_types)
